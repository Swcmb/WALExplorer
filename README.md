...
└──────────┘  └──────────┘  └──────────┘
        ...
        ▼            ▼            ▼
┌──────────┐  ┌──────────┐  ┌──────────┐
│Heap解析器│  │DDL解析器 │  │SQL格式化器│
└──────────┘  └──────────┘  └──────────┘
```

### 核心组件

#### 1. BinaryReader（二进制数据读取器）

负责读取和解析 WAL 文件的二进制数据：

```python
from utils.binary_reader import BinaryReader

reader = BinaryReader(data)
value = reader.read_uint32()  # 读取32位无符号整数
```

#### 2. LSN（日志序列号）

处理 PostgreSQL 的 64 位日志序列号：

```python
from utils.lsn_utils import LSN

lsn = LSN("0/16B37B0")
print(f"文件ID: {lsn.file_id}")
print(f"偏移量: {lsn.file_offset}")
```

#### 3. XLogReader（XLOG 读取器）
...
reader = XLogReader("/path/to/wal/file")
for record in reader.read_records():
    print(f"事务ID: {record.xl_xid}")
```
...
- 项目主页: [https://github.com/Swcmb/WALExplorer](https://github.com/Swcmb/WALExplorer)
- 问题反馈: [Issues](https://github.com/Swcmb/WALExplorer/issues)
- 邮箱: [swcmb@example.com](mailto:swcmb@example.com)
